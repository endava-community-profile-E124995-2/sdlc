...
- Read `SDLC_AUTOMATION_PROJECT_PATHS` from the repo-root `.env` when project-specific context is required.
- Treat the value as a JSON string array of repo-relative paths under `sdlc_automation\projects`.
- Use the listed projects as the default project scope before searching elsewhere.

## Information Hub Context

- Read `SDLC_AUTOMATION_INFORMATION_HUB_PATHS` from the repo-root `.env` when information-hub context is required.
- Treat the value as a JSON string array of repo-relative paths under `sdlc_automation\information_hub`.
- Use the listed information-hub folders as the default shared-context scope before searching elsewhere.
